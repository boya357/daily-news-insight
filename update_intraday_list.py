...
盘中快报列表页自动更新脚本
布局完全固定，只自动扫描并更新报告卡片
用法: python3 update_intraday_list.py
"""
    ...
    <div class="pt-24 pb-8 px-4">
        <div class="max-w-6xl mx-auto text-center">
            <h1 class="text-3xl md:text-4xl font-black text-white mb-3 leading-tight">
                <i class="fa fa-bolt mr-2"></i>盘中快报
            </h1>
            <p class="text-white/80">盘中实时热点与机会捕捉</p>
        </div>
    </div>
...
# 卡片模板（最新报告）
NEWEST_CARD_TEMPLATE = '''<a href="{filename}" class="report-card block p-5 bg-gradient-to-br from-orange-50 to-red-50 border-2 border-orange-200 rounded-xl text-center group hover:shadow-lg transition-all">
    <div class="text-3xl mb-2">⚡</div>
    <div class="font-semibold text-gray-800 text-sm mb-1 group-hover:text-orange-600 transition-colors line-clamp-2">{title}</div>
...
def main():
    ...
    intraday_dir = 'docs/intraday'
    
    # 1. 扫描目录下所有报告文件（排除latest.html）
    all_files = glob.glob(f'{intraday_dir}/20*.html')
    ...
    # 5. 写入文件
    output_path = f'{intraday_dir}/latest.html'
