...
盘后速递列表页自动更新脚本
布局完全固定，只自动扫描并更新报告卡片
用法: python3 update_aftermarket_list.py
"""
    ...
    <div class="pt-24 pb-8 px-4">
        <div class="max-w-6xl mx-auto text-center">
            <h1 class="text-3xl md:text-4xl font-black text-white mb-3 leading-tight">
                <i class="fa fa-sun-o mr-2"></i>盘后速递
            </h1>
            <p class="text-white/80">当日收盘总结与明日策略</p>
        </div>
    </div>
...
# 卡片模板（最新报告）
NEWEST_CARD_TEMPLATE = '''<a href="{filename}" class="report-card block p-5 bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-yellow-200 rounded-xl text-center group hover:shadow-lg transition-all">
    <div class="text-3xl mb-2">☀️</div>
    <div class="font-semibold text-gray-800 text-sm mb-1 group-hover:text-yellow-600 transition-colors line-clamp-2">{title}</div>
...
NORMAL_CARD_TEMPLATE = '''<a href="{filename}" class="report-card block p-5 bg-white border border-gray-100 rounded-xl text-center group hover:shadow-lg transition-all">
    ...
    <span class="inline-block px-2 py-1 text-xs font-bold bg-yellow-100 text-yellow-700 rounded">盘后速递</span>
</a>
'''
...
def main():
    ...
    aftermarket_dir = 'docs/aftermarket'
    
    # 1. 扫描目录下所有报告文件（排除latest.html）
    all_files = glob.glob(f'{aftermarket_dir}/20*.html')
    ...
    # 5. 写入文件
    output_path = f'{aftermarket_dir}/latest.html'
