...
月报列表页自动更新脚本
布局完全固定，只自动扫描并更新报告卡片
用法: python3 update_monthly_list.py
"""
    ...
    <div class="pt-24 pb-8 px-4">
        <div class="max-w-6xl mx-auto text-center">
            <h1 class="text-3xl md:text-4xl font-black text-white mb-3 leading-tight">
                <i class="fa fa-calendar mr-2"></i>月报
            </h1>
            <p class="text-white/80">月度市场总结与策略展望</p>
        </div>
    </div>
...
# 卡片模板（最新报告）
NEWEST_CARD_TEMPLATE = '''<a href="{filename}" class="report-card block p-5 bg-gradient-to-br from-slate-50 to-gray-50 border-2 border-slate-200 rounded-xl text-center group hover:shadow-lg transition-all">
    <div class="text-3xl mb-2">📅</div>
    <div class="font-semibold text-gray-800 text-sm mb-1 group-hover:text-slate-600 transition-colors line-clamp-2">{title}</div>
...
NORMAL_CARD_TEMPLATE = '''<a href="{filename}" class="report-card block p-5 bg-white border border-gray-100 rounded-xl text-center group hover:shadow-lg transition-all">
    ...
    <span class="inline-block px-2 py-1 text-xs font-bold bg-slate-100 text-slate-700 rounded">月报</span>
</a>
'''
...
def main():
    ...
    all_files = glob.glob(f'{monthly_dir}/20*.html')
    ...
    # 5. 写入文件
    output_path = f'{monthly_dir}/latest.html'
