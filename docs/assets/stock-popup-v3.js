 ...
 * 支持自动识别页面中的股票名称
 * 六个维度：技术面、行情面、题材面、基本面、消息面、游资视角
 * 
 * 整合：股票个股分析Skill + 竹石个股Agent Skill
 */
(function() {
    'use strict';
    ...
    function createPopupCard() {
        ...
        card.className = 'stock-popup-card v3';
    ...
    function autoDetectStocks() {
        ...
        function walkTextNodes(node) {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                if (['SCRIPT', 'STYLE', 'CODE', 'PRE', 'TEXTAREA', 'INPUT', 'BUTTON', 'A'].includes(tag)) return;
    ...
    function renderMarketTab(market, overall) {
        if (!market && !overall) {
            return '<div class="tab-content" data-tab="market"><div class="empty-state">暂无行情数据</div></div>';
        }
    ...
    function renderThemeTab(themes) {
        if (!themes || themes.length === 0) {
            return '<div class="tab-content" data-tab="theme"><div class="empty-state">暂无题材数据</div></div>';
        }
    ...
    function renderFundamentalTab(fundamental) {
        if (!fundamental) {
            return '<div class="tab-content" data-tab="fundamental"><div class="empty-state">暂无基本面数据</div></div>';
        }
        ...
        return `
            <div class="tab-content" data-tab="fundamental">
                <div class="metrics-grid">${metricsHtml}</div>
    ...
    // 渲染消息面Tab内容（V3.0 新增）
    function renderNewsTab(newsData) {
        ...
        let newsHtml = '';
        newsData.list.slice(0, 5).forEach(news => {
            const sentiment = news.sentiment || 'neutral';
            const sentimentText = { 'positive': '利好', 'negative': '利空', 'neutral': '中性' }[sentiment];
            const sentimentClass = sentiment;
            
            newsHtml += `
                <div class="news-item">
                    <div class="news-title">${news.title || news.content || '暂无标题'}</div>
                    <div class="news-meta">
                        <span class="news-source">${news.source || '网络'}</span>
        ...
        // 消息面总结
        let impactHtml = '';
        if (newsData.overall_impact || newsData.summary) {
            impactHtml = `
                <div class="news-impact">
                    <div class="news-impact-title">📊 消息面综合判断</div>
                    <div class="news-impact-text">${newsData.overall_impact || newsData.summary}</div>
                </div>
            `;
        }
    ...
    // 渲染游资视角Tab内容（V3.0 新增 - 来自竹石个股Agent）
    function renderTraderViewTab(traderData) {
        if (!traderData || Object.keys(traderData).length === 0) {
            return `
                <div class="tab-content" data-tab="trader">
                    <div class="empty-state">
                        <div style="font-size: 32px; margin-bottom: 10px;">📈</div>
                        <div>游资视角分析生成中...</div>
                    </div>
        ...
        // 催化因素
        let catalystHtml = '';
        if (traderData.catalyst || traderData.core_logic) {
            catalystHtml = `
                <div class="catalyst-box">
                    <div class="catalyst-title">💥 核心催化</div>
                    <div class="catalyst-content">${traderData.catalyst || traderData.core_logic}</div>
                </div>
            `;
        }
        ...
        // 情绪周期
        let emotionHtml = '';
        if (traderData.emotion || traderData.sentiment) {
            const emotion = traderData.emotion || traderData.sentiment;
            const heatPercent = emotion.heat || 50;
            const plateEffect = emotion.plate_effect || '中等';
            const fundsBearing = emotion.funds_bearing || '中等';
            const riskLevel = emotion.risk_level || '中等';
            
            emotionHtml = `
                <div class="emotion-card">
                    <div class="section-title" style="margin-top: 0;">情绪周期</div>
                    <div class="emotion-stats">
                        <div class="emotion-stat-item">
                            <div class="emotion-stat-value hot">${heatPercent}%</div>
                            <div class="emotion-stat-label">市场热度</div>
                            <div class="emotion-bar">
                                <div class="emotion-bar-fill" style="width: ${heatPercent}%; background: linear-gradient(90deg, #3b82f6, #ef4444);"></div>
                            </div>
                        </div>
                        <div class="emotion-stat-item">
                            <div class="emotion-stat-value moderate">${plateEffect}</div>
                            <div class="emotion-stat-label">板块效应</div>
                        </div>
                    </div>
                    <div class="emotion-stats">
                        <div class="emotion-stat-item">
                            <div class="emotion-stat-value cool">${fundsBearing}</div>
                            <div class="emotion-stat-label">资金承接</div>
                        </div>
                        <div class="emotion-stat-item">
                            <div class="emotion-stat-value ${riskLevel === '高' ? 'hot' : 'moderate'}">${riskLevel}</div>
                            <div class="emotion-stat-label">风险等级</div>
                        </div>
                    </div>
                </div>
            `;
        ...
        // 资金博弈
        let fundsHtml = '';
        if (traderData.funds || traderData.capital_flow) {
            const funds = traderData.funds || traderData.capital_flow;
            fundsHtml = `
                <div class="funds-game">
                    <div class="section-title" style="margin-top: 0;">资金博弈</div>
                    <div class="funds-row">
                        <span class="funds-label">主力资金</span>
                        <span class="funds-value ${funds.main_net_inflow ? 'inflow' : 'outflow'}">${funds.main_net_inflow || '---'}</span>
                    </div>
                    <div class="funds-row">
                        <span class="funds-label">游资动向</span>
                        <span class="funds-value ${funds.hot_money_inflow ? 'inflow' : 'outflow'}">${funds.hot_money_inflow || '---'}</span>
                    </div>
                    <div class="funds-row">
                        <span class="funds-label">北向资金</span>
                        <span class="funds-value ${funds.north_bound_inflow ? 'inflow' : 'outflow'}">${funds.north_bound_inflow || '---'}</span>
                    </div>
                    ${funds.longhu ? `
                    <div class="funds-row">
                        <span class="funds-label">龙虎榜</span>
                        <span class="funds-value ${funds.longhu.includes('净买入') ? 'inflow' : 'outflow'}">${funds.longhu}</span>
                    </div>
                    ` : ''}
                </div>
            `;
        ...
        // 操作策略
        let strategyHtml = '';
        if (traderData.strategy || traderData.operation_suggestion) {
            const strategy = traderData.strategy || traderData.operation_suggestion;
            let tagsHtml = '';
            if (strategy.buy_point) tagsHtml += `<span class="strategy-tag">买入: ${strategy.buy_point}</span>`;
            if (strategy.sell_point) tagsHtml += `<span class="strategy-tag warning">卖出: ${strategy.sell_point}</span>`;
            if (strategy.stop_loss) tagsHtml += `<span class="strategy-tag warning">止损: ${strategy.stop_loss}</span>`;
            if (strategy.take_profit) tagsHtml += `<span class="strategy-tag">止盈: ${strategy.take_profit}</span>`;
            
            strategyHtml = `
                <div class="section-title">操作策略</div>
                <div class="strategy-tags">${tagsHtml || '<span class="signal-tag neutral">暂无策略</span>'}</div>
            `;
        }
    ...
    function renderCard(data) {
        ...
        const news = data.news || null;      // 消息面数据（来自股票个股分析Skill）
        const traderView = data.trader || null; // 游资视角数据（来自竹石个股Agent）
        ...
        const tabsHtml = `
            <div class="tabs-nav">
                ...
                <div class="tab-item" data-tab="news">消息面</div>
                <div class="tab-item" data-tab="trader">游资视角</div>
            </div>
        `;
        ...
        const tabsContentHtml = `
            <div class="tabs-content">
                ...
                ${renderMarketTab(market, overall)}
                ${renderThemeTab(themes)}
                ${renderFundamentalTab(fundamental)}
                ${renderNewsTab(news)}
                ${renderTraderViewTab(traderView)}
            </div>
        `;
        ...
        return `
            ...
            <div class="card-footer">
                <a href="${detailUrl}" class="detail-link" target="_blank">查看完整深度分析报告 →</a>
            </div>
        `;
    }
    ...
    window.StockPopup = {
        ...
        updateStockData: function(code, data) {
            stockDataCache[code] = data;
        }
    };
})();
