...
| 工具名称 | 链接路径 | 文件名规范 |
|---------|---------|-----------|
| 每日新闻洞察 | `daily/latest.html` | 使用 latest.html |
| 持仓智能预警仪表盘 | `持仓智能预警仪表盘/index.html` | 使用 index.html |
| 智能选题助手 | `智能选题助手/index.html` | 使用 index.html |
| 预判验证闭环 | `预判验证/index.html` | 使用 index.html |

### 命名规范

- **报告类模块**（daily/intraday/aftermarket/weekly_review等）使用 `latest.html`
- **工具类模块**（仪表盘、选题助手、验证系统等）使用 `index.html`
- **禁止使用不存在的目录名**（如"行业新闻"应改为"daily"）
...
- **2026-05-27**: 首页系统工具模块链接错误
  - 问题1："行业新闻/latest.html" 目录不存在，应改为 "daily/latest.html"
  - 问题2："持仓智能预警仪表盘/latest.html" 应为 "index.html"
  - 问题3："智能选题助手/latest.html" 应为 "index.html"
  - 教训：报告类模块用 latest.html，工具类模块用 index.html；修改链接前先验证目录是否存在
